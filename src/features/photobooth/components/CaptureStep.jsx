...
export default function CaptureStep({
  ...
  return (
    <div className="w-full max-w-6xl flex flex-col lg:flex-row items-center lg:items-start justify-center gap-8 lg:gap-10 relative animate-fade-in px-2 sm:px-4">
      {/* Left Column: Live Camera Preview & Controls */}
      <div className="w-full max-w-xl text-center flex flex-col items-center">
        ...
        <div className="mb-5 flex flex-wrap items-center justify-center gap-3">
          ...
          <button
            ...
            className="px-4 py-2 border-2 border-line bg-white hover:bg-cream rounded-lg text-xs font-bold text-maroon transition-colors shadow-xs cursor-pointer"
          >
            ← Ganti Template
          </button>
        ...
        >
          ...
          {/* Auto-Framing Visual Guide Overlay */}
          <div className="absolute inset-0 pointer-events-none z-10 m-4 flex flex-col justify-between opacity-70 group-hover:opacity-100 transition-opacity duration-300 border-2 border-white/50 border-dashed rounded-2xl">
            <div className="self-start m-2 bg-black/50 backdrop-blur-sm text-white text-[10px] font-mono px-2 py-1 rounded shadow-xs">
              [ AREA CROP AKTIF ]
            </div>
            {/* Crosshairs to help alignment */}
        ...
        <div className="mt-8 w-full max-w-md bg-stone-50 border border-line rounded-2xl p-4 shadow-sm">
          ...
          <div className="flex gap-2.5 justify-center items-center">
            {FILTERS.slice(0, 5).map((f) => {
              ...
              return (
                <button
                  ...
                  className={`w-9 h-9 rounded-xl overflow-hidden border-2 cursor-pointer transition-all hover:scale-[1.05] relative shrink-0 ${
                    isActive
            ...
            <button
              onClick={() => setIsFilterModalOpen(true)}
              className="w-9 h-9 rounded-xl border-2 border-dashed border-stone-400 text-stone-600 hover:border-pink-500 hover:text-pink-600 flex items-center justify-center font-bold text-lg cursor-pointer transition-colors shrink-0"
      ...
      <TemplatePreviewPanel
        ...
        activeSlotIndex={capturingIndex}
      />
